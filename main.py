...
def fix_san_diego_time():
    """Fix San Diego start_time to 9:30 AM PT"""
    if not is_admin_user():
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': f'Updated {san_diego.name} start_time to 9:30 AM PT (deadline for picks: 7:30 AM PT)',
            'competition': {
                'id': san_diego.id,
                'name': san_diego.name,
                'start_time': '09:30',
                'timezone': san_diego.timezone or 'America/Los_Angeles',
                'deadline': '07:30'  # 2 hours before race
            }
        })
    except Exception as e:
        db.session.rollback()
