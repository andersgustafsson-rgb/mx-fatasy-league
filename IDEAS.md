...
| 6c | **Säsongsteam → Combined highscore + promo** | 🛠️ Klart att flippa | **Beslut 25 sep:** Total = Tippa + Säsongsteam (variant A). −50 vid byte stannar på tippa. **Flagor** `SEASON_TEAM_PROMO_LIVE` / `SEASON_TEAM_COMBINED_LIVE`. Promo live nu (räknas ej ännu). **Efter SMX Finals 2026:** `POST /admin/season_teams/archive_and_reset` med `label=SMX 2026` → arkiverar snapshot + nollställer live-lag (alla bygger om inför 2027). Visa antal lag i promo. Combined flippa inför SMX 2027. Ligor = tippa-only. |
