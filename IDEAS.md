...
| 6c | **Säsongsteam → Combined highscore + promo** | 🛠️ Klart att flippa | **Beslut 25 sep:** Total = Tippa + Säsongsteam (variant A). −50 vid byte stannar på tippa. **Flagor** `SEASON_TEAM_PROMO_LIVE` / `SEASON_TEAM_COMBINED_LIVE`. Promo efter helgen; Combined nästa AMA-säsong. Ligor = tippa-only. **UX-idé (25 sep):** När man öppnar sitt **tävlingsresultat** (Mina poäng / race-recap) — visa säsongsteamet med **porträtt + poäng per förare** den tävlingen (ev. totalt tippa+team för racet). API finns redan delvis: `/get_season_team_competition_details/<id>`. |
